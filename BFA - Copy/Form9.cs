...
namespace BFA
{
    public partial class Form9 : Form
    {
        ...
        {
            if (textBox1.Text.Trim() == "" || textBox2.Text.Trim() == "" || comboBox2.Text.Trim() == "" || comboBox5.Text.Trim() == "" || comboBox4.Text.Trim() == "" || comboBox3.Text.Trim() == "" || textBox10.Text.Trim() == "")
            {
            ...
            else
            {
                if (textBox10.Text.Length <= 5)
                {
                    ...
                    string sql = $"SELECT * FROM เลือดร้อยตัวผู้ WHERE ชื่อโค100ตัวผู้ = '{comboBox2.Text}' AND เลขโค100ตัวผู้ = '{comboBox5.Text}'";
                    var cmd = new MySqlCommand(sql, conn);
                    new MySqlDataAdapter(cmd).Fill(dt);
                    if (dt.Rows.Count > 0)
                    {
                        string sql4 = $"SELECT * FROM ลูกผสม3 WHERE ชื่อโค3 = '{comboBox4.Text}' AND เลขโค3 = '{comboBox3.Text}'";
                        var cmd4 = new MySqlCommand(sql4, conn);
                        DataTable dt4 = new DataTable();
                        new MySqlDataAdapter(cmd4).Fill(dt4);
                        if (dt4.Rows.Count > 0)
                        {
                            string sql6 = $"SELECT * FROM ลูกผสม4 WHERE เลขโค4 = '{textBox2.Text+textBox10+textBox11}'";
                            var cmd6 = new MySqlCommand(sql6, conn);
                            DataTable dt6 = new DataTable();
                            new MySqlDataAdapter(cmd6).Fill(dt6);
                            if (dt6.Rows.Count < 1)
                            {
                                String SQL = "INSERT INTO ลูกผสม4 (ชื่อโค4,เลขโค4,ชื่อพ่อโค4,ชื่อแม่โค4,เลขพ่อโค4,เลขแม่โค4,ชื่อเจ้าของ,ชื่อฟาร์ม,สถานะ,day,รับ) VALUES('" + textBox1.Text + "' , '" + textBox2.Text + "' '" + textBox10.Text + "' '" + textBox11.Text + "' , '" + comboBox2.Text + "' , '" + comboBox4.Text + "' , '" + comboBox5.Text + "' , '" + comboBox3.Text + "' , '" + textBox7.Text + "' , '" + textBox8.Text + "' , '" + label13.Text + "' , '" + label14.Text + "' , '" + label15.Text + "')";
        ...
        private void Form9_Shown(object sender, EventArgs e)
        {
            ...
            while (xxxx.Read())
            {
                comboBox5.Items.Add($"{xxxx.GetString(1)}");
                comboBox2.Items.Add($"{xxxx.GetString(0)}");
            }
            conn.Close();

            string sql3 = $"SELECT * FROM ลูกผสม3 WHERE ชื่อฟาร์ม = '{textBox8.Text}'";
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            while (xxxx.Read())
            {
                comboBox5.Text = xxxx.GetString(1);
            }
            conn.Close();
        }
        ...
        {
            string sql4 = "SELECT * FROM ลูกผสม3 WHERE เลขโค3 = '" + comboBox3.Text + "' ";
            MySqlConnection conn = new MySqlConnection("host=localhost;user=root;password=;database=สมาคมฟาร์มโค");
        ...
        {
            string sql4 = "SELECT * FROM ลูกผสม3 WHERE ชื่อโค3 = '" + comboBox4.Text + "' ";
            MySqlConnection conn = new MySqlConnection("host=localhost;user=root;password=;database=สมาคมฟาร์มโค");
            MySqlCommand cmd6 = new MySqlCommand(sql4, conn);
            conn.Open();
            DataTable dt6 = new DataTable();
