...
namespace BFA
{
    public partial class Form5 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (textBox10.Text.Length <= 5)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        string sql4 = $"SELECT * FROM ลูกผสม WHERE ชื่อโค = '{comboBox4.Text}' AND เลขโค = '{comboBox3.Text}'";
                        var cmd4 = new MySqlCommand(sql4, conn);
                        DataTable dt4 = new DataTable();
                        new MySqlDataAdapter(cmd4).Fill(dt4);
                        if (dt4.Rows.Count > 0)
                        {
                            string sql6 = $"SELECT * FROM ลูกผสม2 WHERE เลขโค2 = '{textBox2.Text+textBox10.Text+textBox11.Text}'";
                            var cmd6 = new MySqlCommand(sql6, conn);
                            DataTable dt6 = new DataTable();
                            new MySqlDataAdapter(cmd6).Fill(dt6);
                            if (dt6.Rows.Count < 1)
                            {
                                String SQL = "INSERT INTO ลูกผสม2 (ชื่อโค2,เลขโค2,ชื่อพ่อโค2,ชื่อแม่โค2,เลขพ่อโค2,เลขแม่โค2,ชื่อเจ้าของ,ชื่อฟาร์ม,สถานะ,day,รับ) VALUES('" + textBox1.Text + "' , '" + textBox2.Text + "' '" + textBox10.Text + "' '" + textBox11.Text + "' , '" + comboBox2.Text + "' , '" + comboBox4.Text + "' , '" + comboBox1.Text + "' , '" + comboBox3.Text + "' , '" + textBox7.Text + "' , '" + textBox8.Text + "' , '" + label13.Text + "' , '" + label14.Text + "' , '" + label15.Text + "')";
        ...
        private void Form5_Shown(object sender, EventArgs e)
        {
            ...
            string sql3 = $"SELECT * FROM ลูกผสม WHERE ชื่อฟาร์ม = '{textBox8.Text}'";
