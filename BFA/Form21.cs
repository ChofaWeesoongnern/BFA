...
namespace BFA
{
    public partial class Form21 : Form
    {
        ...
        private void label6_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection conn = new MySqlConnection("host=localhost;user=root;password=;database=สมาคมฟาร์มโค");
                string sql = $"SELECT * FROM ลูกผสม2 WHERE เลขโค2 = '{label6.Text}' ";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                conn.Open();
                DataTable dt = new DataTable();
                new MySqlDataAdapter(cmd).Fill(dt);
                ...
                string sql1 = $"SELECT * FROM ลูกผสม2 WHERE เลขโค2 = '{label6.Text}' ";
                MySqlCommand cmd1 = new MySqlCommand(sql1, conn);
                conn.Open();
                DataTable dt1 = new DataTable();
                new MySqlDataAdapter(cmd1).Fill(dt1);
        ...
        private void label8_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection conn = new MySqlConnection("host=localhost;user=root;password=;database=สมาคมฟาร์มโค");
                string sql = $"SELECT * FROM ลูกผสม WHERE เลขโค = '{label8.Text}' ";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                conn.Open();
                DataTable dt = new DataTable();
                new MySqlDataAdapter(cmd).Fill(dt);
                ...
                string sql1 = $"SELECT * FROM ลูกผสม WHERE เลขโค = '{label8.Text}' ";
                MySqlCommand cmd1 = new MySqlCommand(sql1, conn);
                conn.Open();
                DataTable dt1 = new DataTable();
                new MySqlDataAdapter(cmd1).Fill(dt1);
